...
import { TrashList } from '@/app/(app)/server/[uuidShort]/files/components/trash/TrashList';
import { TrashActionToolbar } from '@/app/(app)/server/[uuidShort]/files/components/trash/TrashActionToolbar';
...
export default function WebSpaceTrashPage({ params }: { params: Promise<{ uuidShort: string }> }) {
    ...
    const refresh = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const data = await webspaceFilesApi.listTrash(uuidShort);
    ...
    const handleRestore = async (overwrite: boolean) => {
        ...
        setBusy(true);
        try {
            await webspaceFilesApi.restoreTrash(uuidShort, selected, overwrite);
    ...
    const handleEmpty = async () => {
        setBusy(true);
        try {
            await webspaceFilesApi.emptyTrash(uuidShort);
    ...
    if (!trashEnabled) {
        return (
            <WebSpacePageWidgets pageId='webspace-files-trash'>
                <div className='relative flex min-h-screen flex-col gap-6 pb-20'>
                    ...
                    <Button asChild variant='outline'>
                        <Link href={`/webspace/${uuidShort}/files`}>
        ...
        <WebSpacePageWidgets pageId='webspace-files-trash'>
            <div className='relative flex min-h-screen flex-col gap-6 pb-20'>
                ...
                <TrashActionToolbar
                    ...
                    filesHref={`/webspace/${uuidShort}/files`}
                    loading={loading}
