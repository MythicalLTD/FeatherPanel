...
import { ListCheck, Plus, Lock } from 'lucide-react';
...
import type { Database, TaskCreateRequest } from '@/types/server';
...
export default function CreateTaskPage() {
    ...
    const [form, setForm] = React.useState<TaskCreateRequest>({
        action: 'backup',
        payload: '',
        time_offset: 0,
    ...
    const handleCreate = async (e?: React.FormEvent) => {
        ...
        setSaving(true);
        try {
            const { data } = await axios.post(`/api/user/servers/${uuidShort}/schedules/${scheduleId}/tasks`, {
                ...form,
                payload,
            });
            if (data?.success) {
    ...
    return (
        <div className='mx-auto max-w-4xl space-y-8 pb-16'>
            <WidgetRenderer widgets={getWidgets('server-tasks-new', 'top-of-page')} />
            <PageHeader
                    ...
                    {form.action === 'power' ? (
                        <div className='space-y-2.5'>
                            ...
                            <p className='text-muted-foreground ml-1 text-xs'>
                                {t('serverTasks.selectPowerActionHelp')}
                            </p>
                        </div>
                    ) : form.action === 'backup' ? (
                        <BackupTaskFields
                        ...
                        <div className='space-y-2.5'>
                            ...
                            <p className='text-muted-foreground ml-1 text-xs'>
                                {form.action === 'command'
                                    ? t('serverTasks.commandHelp')
                                    : t('serverTasks.additionalDataHelp')}
                            </p>
                        </div>
                    )}
                </FormSection>
            ...
            <WidgetRenderer widgets={getWidgets('server-tasks-new', 'bottom-of-page')} />
        </div>
    );
}
