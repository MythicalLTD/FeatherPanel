...
import { DatabaseDumpsPanel } from '@/components/server/backup/DatabaseDumpsPanel';
...
export default function ServerBackupsPage() {
    ...
    const activeTab: BackupTab = tabParam === 'databases' ? 'databases' : 'files';

    const setActiveTab = (tab: BackupTab) => {
        ...
        router.replace(qs ? `/server/${uuidShort}/backups?${qs}` : `/server/${uuidShort}/backups`, { scroll: false });
    };
    ...
    const deletableBackups = backups.filter((b) => b.is_locked !== 1);
    const allDeletableSelected =
        deletableBackups.length > 0 && deletableBackups.every((b) => selectedUuids.includes(b.uuid));

    const toggleBackupSelection = (uuid: string) => {
    ...
    const toggleSelectAllDeletable = () => {
        if (allDeletableSelected) {
            ...
            setSelectedUuids(deletableBackups.map((b) => b.uuid));
        }
    };
    ...
    const handleBulkDeleteResult = (data: {
        ...
        if (skipped > 0 || failed > 0) {
            toast.success(
                t('serverBackups.bulkDeletePartial', {
            ...
            toast.success(t('serverBackups.bulkDeleteSuccess', { count: String(deleted) }));
        }
        setSelectedUuids([]);
        fetchBackups();
    };
    ...
    const handleDeleteSelected = () => {
        ...
        setConfirmAction({
            title: t('serverBackups.confirmBulkDeleteTitle'),
            description: t('serverBackups.confirmBulkDeleteDescription', {
                count: String(selectedUuids.length),
            }),
            variant: 'destructive',
            action: async () => {
                try {
                    const { data } = await axios.delete(`/api/user/servers/${uuidShort}/backups/bulk-delete`, {
    ...
    const handleDeleteAll = () => {
        setConfirmAction({
            title: t('serverBackups.confirmDeleteAllTitle'),
            description: t('serverBackups.confirmDeleteAllDescription'),
            variant: 'destructive',
            action: async () => {
                try {
                    const { data } = await axios.delete(`/api/user/servers/${uuidShort}/backups/bulk-delete`, {
    ...
    return (
        <div className='space-y-8 pb-12'>
                    ...
                    <div className='flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center sm:gap-3'>
                        {showHeaderCreateAction && (
                            <Button
                                size='default'
                                disabled={loading}
                                onClick={() => router.push(`/server/${uuidShort}/backups/new`)}
            ...
            <Tabs
                value={activeTab}
                onValueChange={(value) => setActiveTab(value === 'databases' ? 'databases' : 'files')}
                className='space-y-6'
            >
                <TabsList className='bg-card/50 border-border/50 grid h-auto w-full grid-cols-2 gap-1 rounded-2xl border p-1.5'>
                    <TabsTrigger
                        value='files'
                        className='data-[state=active]:bg-primary data-[state=active]:text-primary-foreground flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-bold'
                    >
                        <HardDrive className='h-4 w-4' />
                        {t('serverBackups.tabFiles')}
                    </TabsTrigger>
                    <TabsTrigger
                        value='databases'
                        className='data-[state=active]:bg-primary data-[state=active]:text-primary-foreground flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-bold'
                ...
                <TabsContent value='files' className='mt-0 space-y-6'>
                        ...
                        <div className='border-border bg-card/50 flex flex-wrap items-center justify-between gap-3 rounded-xl border px-4 py-3'>
                            <div className='flex items-center gap-3'>
                                <Checkbox
                                    ...
                                    disabled={deletableBackups.length === 0}
                                    aria-label={t('serverBackups.selectAll')}
                                />
                                <span className='text-sm font-medium'>
                                    {selectedUuids.length > 0
                                        ? t('serverBackups.selectedCount', { count: String(selectedUuids.length) })
                            ...
                            {selectedUuids.length > 0 && (
                                ...
                                >
                                    <Trash2 className='mr-2 h-4 w-4' />
                                    {t('serverBackups.deleteSelected')} ({selectedUuids.length})
                                </Button>
                            )}
                        </div>
                    )}
                    ...
                    {backups.length === 0 ? (
                        ...
                        <div className='grid grid-cols-1 gap-4'>
                            {backups.map((backup) => (
                                <div key={backup.id} className='flex items-stretch gap-3'>
                                    {canDelete && (
                                        <div className='flex shrink-0 items-center pl-1'>
                                            <Checkbox
                                                checked={selectedUuids.includes(backup.uuid)}
                                                disabled={backup.is_locked === 1}
                                                onCheckedChange={() => toggleBackupSelection(backup.uuid)}
                                    ...
                                    <ResourceCard
                                        className={cn(
                                            ...
                                            !backup.completed_at &&
                                                ...
                                                'animate-pulse border-blue-500/20',
                                            selectedUuids.includes(backup.uuid) && 'border-primary/40 bg-primary/5',
                                            ...
                                            (canRestore || canDownload || canDelete) && (
                                                <DropdownMenu>
                                                    ...
                                                    >
                                                        ...
                                                        <DropdownMenuItem
                                                            onClick={() =>
                                                                handleLockBackup(backup, backup.is_locked === 0)
                                                            }
                                                            className='flex cursor-pointer items-center gap-3 rounded-xl p-3'
                                                        >
                                                            {backup.is_locked === 1 ? (
                ...
                <TabsContent value='databases' className='mt-0'>
                    <DatabaseDumpsPanel
                        uuidShort={uuidShort}
                        canCreate={canCreate}
                        onCreate={() => router.push(`/server/${uuidShort}/backups/new?kind=database`)}
                    />
                </TabsContent>
            </Tabs>
            ...
            <ConfirmDialog
                ...
                title={
                    <span className='flex items-center gap-4'>
                        ...
                        <span className='text-xl leading-none font-bold'>{confirmAction?.title}</span>
                    </span>
                }
                description={confirmAction?.description}
                cancelLabel={t('common.cancel')}
                confirmLabel={actionLoading ? <Loader2 className='h-5 w-5 animate-spin' /> : t('common.confirm')}
