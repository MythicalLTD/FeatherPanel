...
export default function EditWebPlatePage() {
    const { t } = useTranslation();
    const router = useRouter();
    const params = useParams();
    const id = Number(params?.id);
    const { fetchWidgets, getWidgets } = usePluginWidgets('admin-webplates-edit');
    ...
    useEffect(() => {
        ...
        const load = async () => {
            setLoading(true);
            try {
                const { data } = await axios.get(`/api/admin/webplates/${id}`);
                const plate = (data.data?.webplate ?? data.data) as WebPlate;
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            await axios.patch(`/api/admin/webplates/${id}`, webPlateFormPayload(form));
            toast.success(t('admin.webPlates.messages.updated'));
            router.push('/admin/webplates');
        } catch (error) {
            console.error(error);
            toast.error(getApiErrorMessage(error, t, 'admin.webPlates.messages.update_failed'));
        } finally {
            setSaving(false);
        }
    };
    ...
    if (loading) {
        return (
            <div className='space-y-6'>
                <WidgetRenderer widgets={getWidgets('admin-webplates-edit', 'top-of-page')} />
                <PageHeader
                    title={t('admin.webPlates.form.edit_title')}
                    description={t('admin.webPlates.form.edit_description')}
                    icon={LayoutTemplate}
                />
    ...
    return (
        <div className='space-y-6'>
            ...
            <WidgetRenderer widgets={getWidgets('admin-webplates-edit', 'after-header')} />
            ...
            <WidgetRenderer widgets={getWidgets('admin-webplates-edit', 'bottom-of-page')} />
        </div>
    );
}
