            ...
            <PageCard title={t('admin.vmInstances.edit_tabs.network') ?? 'Network'} icon={Wifi}>
                <div className='space-y-4'>
                    {isLxc ? (
                        ...
                        <>
                            <div>
                                <Label>{t('admin.vmInstances.current_ip') ?? 'Current IP'}</Label>
                                <p className='text-foreground font-semibold text-base tabular-nums mt-1'>
                                    {(instance?.ip_address as string) ?? '—'}
                                </p>
                            </div>
                            <div>
                                <Label>{t('admin.vmInstances.change_ip') ?? 'Change IP'}</Label>
                                <HeadlessSelect
                                    value={vmIpId ?? ''}
                                    onChange={(v) => setVmIpId(v === '' ? null : Number(v))}
                                    options={ipOptionsQemu.map((ip) => ({
                                        id: ip.id,
                                        name: instance?.vm_ip_id === ip.id ? `${ip.ip} (current)` : ip.ip,
                                    }))}
                                    placeholder='— Change IP —'
