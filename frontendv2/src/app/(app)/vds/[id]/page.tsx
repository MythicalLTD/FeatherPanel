...
const vmStatusStyles: Record<string, { badge: string; dot: string; label: string }> = {
    running: { badge: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30', dot: 'bg-emerald-400', label: 'Running' },
    stopped: { badge: 'bg-red-500/15 text-red-400 border-red-500/30', dot: 'bg-red-400', label: 'Stopped' },
    starting: { badge: 'bg-blue-500/15 text-blue-400 border-blue-500/30', dot: 'bg-blue-400 animate-pulse', label: 'Starting' },
    stopping: { badge: 'bg-orange-500/15 text-orange-400 border-orange-500/30', dot: 'bg-orange-400 animate-pulse', label: 'Stopping' },
    suspended: { badge: 'bg-amber-500/15 text-amber-400 border-amber-500/30', dot: 'bg-amber-400', label: 'Suspended' },
    creating: { badge: 'bg-blue-500/15 text-blue-400 border-blue-500/30', dot: 'bg-blue-400 animate-pulse', label: 'Creating' },
    unknown: { badge: 'bg-muted/50 text-muted-foreground border-border/30', dot: 'bg-muted-foreground', label: 'Unknown' },
};

function StatusBadge({ status }: { status: string }) {
    const s = vmStatusStyles[status] ?? vmStatusStyles.unknown;
    return (
        <span className={cn('inline-flex items-center gap-2 px-3 py-1 text-sm font-semibold rounded-full border', s.badge)}>
...
function StatCard({
    ...
    const colorMap: Record<string, string> = {
        ...
        emerald: 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20 glow-emerald',
        blue: 'text-blue-400 bg-blue-400/10 border-blue-400/20 glow-blue',
        amber: 'text-amber-400 bg-amber-400/10 border-amber-400/20 glow-amber',
        violet: 'text-violet-400 bg-violet-400/10 border-violet-400/20 glow-violet',
        cyan: 'text-cyan-400 bg-cyan-400/10 border-cyan-400/20 glow-cyan',
        indigo: 'text-indigo-400 bg-indigo-400/10 border-indigo-400/20 glow-indigo',
        purple: 'text-purple-400 bg-purple-400/10 border-purple-400/20 glow-purple',
    } as Record<string, string>;

    return (
        <div className='group relative overflow-hidden rounded-4xl border border-white/5 bg-[#0a0a0b]/40 backdrop-blur-2xl p-6 transition-all hover:bg-[#0a0a0b]/60 animate-in fade-in zoom-in duration-500'>
            <div className='flex items-center gap-5 relative z-10'>
                <div className={cn(
                    'h-14 w-14 rounded-2xl flex items-center justify-center border transition-all duration-500 group-hover:scale-110 shadow-2xl',
                ...
                <div className='flex flex-col space-y-1'>
                    ...
                    <div className='flex items-baseline gap-2'>
                        ...
                        {sub && (
                            <span className='text-[10px] font-black text-white/20 italic uppercase tracking-widest'>
                                {sub}
                            </span>
                        )}
                    </div>
            ...
            <div 
                ...
                style={{ backgroundColor: colorMap[color]?.split(' ')[0].replace('text-', '') }}
            />
        </div>
    );
}
    ...
    const fetchStatus = useCallback(async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                setMemoryData(prev => [...prev.slice(-maxDataPoints + 1), { timestamp: now, value: status.mem ?? 0 }]);
                ...
                if (prevStatsRef.current && status.netin != null && status.netout != null) {
                    ...
                    if (timeDiff > 0) {
                        ...
                        setNetworkRxData(prev => [...prev.slice(-maxDataPoints + 1), { timestamp: now, value: rxDiff / timeDiff }]);
                        setNetworkTxData(prev => [...prev.slice(-maxDataPoints + 1), { timestamp: now, value: txDiff / timeDiff }]);
                    }
                } else {
                ...
                actions={
                    <div className='flex items-center gap-2 flex-wrap'>
                        <Button
                            variant='glass'
                            size='sm'
                            onClick={() => { fetchStatus(); refreshInstance(); }}
                ...
                <Card className='border-border/20 bg-card/30 backdrop-blur-sm'>
                    ...
                    <CardContent className='space-y-3'>
                        {[
                            ...
                            <div key={label} className='flex items-center justify-between py-2 border-b border-border/10 last:border-0'>
                    ...
                    <CardContent className='flex flex-col items-center justify-center py-12 gap-6 text-center'>
                        {!canConsole ? (
                            ...
                            <>
                                ...
                                {canPower && (
                                    <Button onClick={() => handlePower('start')} disabled={powering !== null} className='mt-2'>
                                        {powering === 'start' ? (
