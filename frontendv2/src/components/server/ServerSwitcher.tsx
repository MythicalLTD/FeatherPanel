...
function ServerAvatar({ server, className }: { server: Server; className?: string }) {
    ...
    if (banner) {
        return (
            <div
                className={cn('shrink-0 overflow-hidden rounded-md bg-cover bg-center ring-1 ring-border/40', className)}
    ...
    return (
        <div
            className={cn(
                'bg-primary/15 text-primary flex shrink-0 items-center justify-center rounded-md text-xs font-bold ring-1 ring-primary/20',
                className,
            )}
            aria-hidden='true'
        >
...
export function ServerSwitcher({ fallbackTitle }: ServerSwitcherProps) {
    ...
    const sortedServers = useMemo(
        () => sortServersForSwitcher(servers, currentUuidShort),
        [servers, currentUuidShort],
    );
        ...
        <Menu as='div' className='relative min-w-0 max-w-full'>
            <MenuButton
                ...
                className={cn(
                    ...
                    'hover:bg-muted/40 focus-visible:ring-2 focus-visible:ring-primary/40',
                )}
            ...
            >
                {loading && !currentServer ? (
                    ...
                    <div className='bg-muted/40 flex h-7 w-7 shrink-0 items-center justify-center rounded-md ring-1 ring-border/40 sm:h-8 sm:w-8'>
                        ...
                        >
                            {tabs.map((tab) => {
                                ...
                                return (
                                    ...
                                    >
                                        {Icon && <Icon className={cn('h-3 w-3 shrink-0', isActive && 'fill-current')} />}
