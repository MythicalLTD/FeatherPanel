...
export default function PlasmaWave(props: PlasmaWaveProps) {
    ...
    useEffect(() => {
        ...
        const uniformOffset = new Float32Array([initial.xOffset ?? 0, initial.yOffset ?? 0]);
        const uniformResolution = new Float32Array([1, 1]);
        const c1 = hexToRgb(initial.colors?.[0] ?? '#A855F7');
        const c2 = hexToRgb(initial.colors?.[1] ?? '#06B6D4');
