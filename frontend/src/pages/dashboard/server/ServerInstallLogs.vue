    ...
    <DashboardLayout :breadcrumbs="breadcrumbs">
        <div class="space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold">{{ t('serverInstallLogs.title') }}</h1>
                    <p class="text-muted-foreground">{{ t('serverInstallLogs.description') }}</p>
                </div>
                <div class="flex gap-2">
                    <Button variant="outline" :disabled="loading" @click="refreshLogs">
                        <RefreshCw class="h-4 w-4 mr-2" />
                        {{ t('serverInstallLogs.refresh') }}
                    </Button>
                    <Button variant="outline" :disabled="loading" @click="downloadLogs">
                        <Download class="h-4 w-4 mr-2" />
                        {{ t('serverInstallLogs.download') }}
                    </Button>
                </div>
            </div>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription> {{ t('serverInstallLogs.lastUpdated') }}: {{ lastUpdated }} </CardDescription>
                </CardHeader>
                <CardContent>
                    <div v-if="loading" class="flex items-center justify-center py-8">
                        ...
                        ></div>
                        <span class="ml-2">{{ t('serverInstallLogs.loading') }}</span>
                    </div>

                    <div v-else-if="logs.length === 0" class="text-center py-8 text-muted-foreground">
                        {{ t('serverInstallLogs.noLogs') }}
                    </div>
...
const breadcrumbs = computed(() => [
    ...
    { text: 'Install Logs', isCurrent: true, href: `/server/${route.params.uuidShort}/logs/install` },
]);
