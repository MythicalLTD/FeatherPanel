...
export function useWingsWebSocket({
  ...
  const refreshToken = useCallback(async () => {
    try {
      ...
      const response = await axios.post<WingsJWTResponse>(
        `/api/user/servers/${serverUuid}/jwt`
      );
    ...
    const connect = async () => {
      ...
      try {
        ...
        if (!response.data.success) {
          throw new Error(
            response.data.error_message || "Failed to get JWT token"
          );
        }
          ...
          try {
            ...
            if (data.event === "stats") {
              ...
              if (lastStatsRequestTimeRef.current !== null) {
                const roundTripTime =
                  Date.now() - lastStatsRequestTimeRef.current;
