...
export default function EditSchedulePage() {
    ...
    React.useEffect(() => {
        const fetchSchedule = async () => {
            ...
            setLoading(true)
            try {
                const { data } = await axios.get<{success: boolean, data: Schedule}>(`/api/user/servers/${uuidShort}/schedules/${id}`)
                ...
                <div className="space-y-3">
                    ...
                    <div className="space-y-1">
                        <div className="flex items-center gap-3">
                            ...
                            <h1 className="text-3xl font-black tracking-tight uppercase italic leading-none">{t("serverSchedules.editSchedule")}</h1>
                        </div>
                        <p className="text-sm text-muted-foreground font-medium opacity-60 ml-15 max-w-xl">
                            {t("serverSchedules.editScheduleDescription")}
                        </p>
                    </div>
                </div>
