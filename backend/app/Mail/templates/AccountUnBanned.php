...
class AccountUnBanned
{
    ...
    public static function getTemplate(array $data): string
    {
        if (isset($data['app_name']) && isset($data['app_url']) && isset($data['first_name']) && isset($data['last_name']) && isset($data['email']) && isset($data['username']) && isset($data['app_support_url'])) {
            return self::parseTemplate(MailTemplate::getByName('account_unsuspended')['body'] ?? '', [
                ...
                'unsuspend_time' => $data['unsuspend_time'],
            ]);
        }
    ...
    public static function parseTemplate(string $template, array $data): string
    {
        ...
        $template = str_replace('{unsuspension_time}', $data['unsuspension_time'], $template);
    ...
    public static function send(array $data): void
    {
        if (
            ...
            || !isset($data['uuid'])
            || !isset($data['enabled'])
            || !isset($data['unsuspend_time'])
        ) {
            return;
        }

        if ($data['enabled'] == 'false' || $data['unsuspend_time'] == '') {
            return;
        }
    ...
    {
        $row = MailTemplate::getByName('account_unsuspended');
        ...
        return self::parseTemplate($subjectTemplate, [
            ...
            'dashboard_url' => $data['app_url'] . '/dashboard',
            'support_url' => $data['app_support_url'],
            'unsuspension_time' => $data['unsuspend_time'],
        ]);
    }
}
