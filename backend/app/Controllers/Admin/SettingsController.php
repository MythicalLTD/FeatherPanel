...
class SettingsController
{
    ...
    private $settingsCategories = [
        'app' => [
            ...
            'settings' => [
                ...
                ConfigInterface::VNC_PROXY_VIA_PANEL,
                ConfigInterface::VNC_USE_PVE_REDIRECT,
    ...
    public function __construct()
    {
        ...
        $this->settings = [
            ...
            ConfigInterface::VNC_PROXY_VIA_PANEL => [
                'name' => ConfigInterface::VNC_PROXY_VIA_PANEL,
                'value' => $this->app->getConfig()->getSetting(ConfigInterface::VNC_PROXY_VIA_PANEL, 'false'),
                'description' => 'Route VNC console through the panel URL (recommended if the browser cannot reach the Proxmox host directly). When enabled, the reverse proxy must forward /vnc-proxy/ to your Proxmox API (e.g. https://virt.mythical.systems:8006). See docs or nginx example in backend docs.',
                'type' => 'select',
                'required' => true,
                'placeholder' => 'false',
            ...
            ConfigInterface::VNC_USE_PVE_REDIRECT => [
                'name' => ConfigInterface::VNC_USE_PVE_REDIRECT,
                'value' => $this->app->getConfig()->getSetting(ConfigInterface::VNC_USE_PVE_REDIRECT, 'false'),
                'description' => 'When enabled, create a short-lived Proxmox user with console-only access and return a Proxmox noVNC URL (token in query). Frontend will redirect to Proxmox instead of embedding noVNC. Requires User.Modify and ACL.Modify on the node.',
                'type' => 'select',
                'required' => true,
                'placeholder' => 'false',
                'validation' => 'required|string|in:true,false',
