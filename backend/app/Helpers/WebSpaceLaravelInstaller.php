...
/**
 * One-click Laravel install into a WebSpace subdirectory.
 */
class WebSpaceLaravelInstaller
{
    /**
    ...
    {
        WebSpaceAppsCatalog::requireApp($space, WebSpaceAppsCatalog::APP_LARAVEL);

        $runtime = self::runtime($space);
        if ($runtime !== 'php') {
            throw new \InvalidArgumentException('Laravel requires a PHP WebSpace');
        }

        $directory = self::normalizeDirectory((string) ($input['directory'] ?? '/'));
        $appName = trim((string) ($input['app_name'] ?? 'Laravel'));
        if ($appName === '') {
            $appName = 'Laravel';
        }
        ...
        $db = self::provisionDatabase($space, array_merge($input, [
            'database_name' => (string) ($input['database_name'] ?? 'laravel'),
        ]));
        ...
        $cmd = 'mkdir -p ' . self::shellQuote($parentPath)
            ...
            . ' && composer create-project laravel/laravel ' . self::shellQuote(basename($containerPath))
            . ' --no-interaction --prefer-dist'
            . ' && cd ' . self::shellQuote($containerPath)
            . ' && sed -i ' . self::shellQuote('s/^APP_NAME=.*/APP_NAME=' . $appName . '/')
            . ' .env'
            . ' && sed -i ' . self::shellQuote('s|^APP_URL=.*|APP_URL=' . $url . '|')
            . ' .env'
            . ' && sed -i ' . self::shellQuote('s/^DB_DATABASE=.*/DB_DATABASE=' . $db['database'] . '/')
            . ' .env'
            . ' && sed -i ' . self::shellQuote('s/^DB_USERNAME=.*/DB_USERNAME=' . $db['username'] . '/')
            . ' .env'
            . ' && sed -i ' . self::shellQuote('s/^DB_PASSWORD=.*/DB_PASSWORD=' . $db['password'] . '/')
            . ' .env'
            . ' && sed -i ' . self::shellQuote('s/^DB_HOST=.*/DB_HOST=' . $dbHost . '/')
            . ' .env'
            . ' && php artisan key:generate --force'
            . ' && php artisan migrate --force --no-interaction';

        $output = self::runExec($webNode, $uuid, $cmd, 600, 'Laravel install failed');
    ...
    private static function provisionDatabase(array $space, array $input): array
    {
        ...
        $namePart = preg_replace('/[^a-zA-Z0-9_]/', '', (string) ($input['database_name'] ?? 'laravel')) ?: 'laravel';
        $databaseName = 'l' . $spaceId . '_' . substr($namePart, 0, 24);
