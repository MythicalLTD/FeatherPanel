...
/**
 * One-click Joomla install: download stable package and provision a database.
 */
class WebSpaceJoomlaInstaller
{
    private const JOOMLA_ZIP = 'https://github.com/joomla/joomla-cms/releases/download/5.2.3/Joomla_5.2.3-Stable-Full_Package.zip';
    ...
    {
        WebSpaceAppsCatalog::requireApp($space, WebSpaceAppsCatalog::APP_JOOMLA);

        $runtime = self::runtime($space);
        if ($runtime !== 'php') {
            throw new \InvalidArgumentException('Joomla requires a PHP WebSpace');
        }
        ...
        if (!$pull['ok']) {
            throw new \RuntimeException($pull['error'] ?? 'Failed to download Joomla');
        }

        $zipPath = rtrim($directory, '/') . '/joomla.zip';
        $decompress = FeatherQuilldClient::decompressWebSpaceFile($webNode, $uuid, $zipPath, $directory);
        if (!$decompress['ok']) {
            throw new \RuntimeException($decompress['error'] ?? 'Failed to extract Joomla');
        }

        $cmd = 'cd ' . self::shellQuote($containerPath)
            . ' && rm -f joomla.zip';

        $output = self::runExec($webNode, $uuid, $cmd, 120, 'Joomla file setup failed');

        $db = self::provisionDatabase($space, array_merge($input, [
            'database_name' => (string) ($input['database_name'] ?? 'joomla'),
        ]));
        ...
        return [
            ...
            'output' => $output,
            'setup_note' => 'Complete the Joomla web installer using the database credentials above.',
        ];
    }
    ...
    private static function provisionDatabase(array $space, array $input): array
    {
        ...
        $namePart = preg_replace('/[^a-zA-Z0-9_]/', '', (string) ($input['database_name'] ?? 'joomla')) ?: 'joomla';
        $databaseName = 'j' . $spaceId . '_' . substr($namePart, 0, 24);
